...
def plot_fms_diff_ranks_dev_norm(
    X_dev: anndata.AnnData,
    X_lognorm: anndata.AnnData,
    ax: Axes,
    ranksList: list[int],
    runs: int,
):
    ...
    for j in range(0, runs, 1):
        scores = []
        for i in ranksList:
            devPf2 = pf2(X_dev, rank=i, random_state=j, doEmbedding=False)

            normPf2 = pf2(X_lognorm, rank=i, random_state=j, doEmbedding=False)

            fmsScore = calculateFMS(devPf2, normPf2)
            scores.append(fmsScore)
        fmsLists.append(scores)
