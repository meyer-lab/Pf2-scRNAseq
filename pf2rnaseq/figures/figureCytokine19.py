...
def suppressive_score(X: anndata.AnnData):
    ...
    immune_suppressive = ["FOXP3","TIGIT","ICOS","IL2RA","TGFB1","SOCS3","BCL2","TNFRSF18"]  

    X = sc.tl.score_genes(adata=X, gene_list=immune_suppressive, copy=True, use_raw=False)
...
def plot_score(X: anndata.AnnData, ax: Axes, cellType: str = "Cell Type"):
    ...
    ax.legend(handles[:len(df["Condition"].unique())], labels[:len(df["Condition"].unique())])
