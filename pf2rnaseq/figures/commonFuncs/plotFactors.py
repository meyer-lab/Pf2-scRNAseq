...
def plot_condition_factors_groups(
    ...
    if cond_group_labels is not None:
        ...
        ax.tick_params(axis="y", which="major", pad=40 if subgroup_labels is not None else 20, length=0)
        
        # Main group colors - use a distinct palette
        main_colors = sns.color_palette("tab10", 
            n_colors=pd.Series(cond_group_labels).nunique()
        ).as_hex()
        main_lut = {}
        ...
        if subgroup_labels is not None:
            ...
            ax.legend(handles=main_legend_elements, bbox_to_anchor=(0, 1.3), title=main_group_title)
...
def plot_gene_factors(data: AnnData, ax: Axes, trim=True): #yt gene names- input that will 
...
def plot_geneSet_factors(data: AnnData, ax: Axes, genes:np.array , trim=True): #yt gene names- input that will 
...
def plot_geneSetScore(data: AnnData, ax: Axes, genes:np.array , trim=True): #yt gene names- input that will 
...
def plot_geneSetScoreDot(data: AnnData, ax: Axes, genes: np.array, trim=True, 
                      size_scale=500, cmap="coolwarm", size_norm=None):
    """
    ...
    kept_idxs = np.where(np.in1d(yt, genes))[0]
    if len(kept_idxs) == 0:
        ax.text(0.5, 0.5, "No matching genes found", ha='center', va='center', transform=ax.transAxes)
        return
